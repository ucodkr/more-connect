...
const FILE_NAME = "more-connect-vscode-favorites.json";
const CONFIG_NAME = "connections-storage.json";

export class VsCodeFavoriteStore {
  private items: VsCodeFavorite[] = [];
  ...
  public list(): VsCodeFavorite[] {
    return this.items;
  }

  public async saveAll(items: VsCodeFavorite[]): Promise<void> {
    this.items = items;
  ...
  private async readFile(folder: vscode.Uri): Promise<VsCodeFavorite[] | undefined> {
    try {
      const bytes = await vscode.workspace.fs.readFile(this.fileUri(folder));
      const text = Buffer.from(bytes).toString("utf8");
      const parsed = JSON.parse(text);
      return Array.isArray(parsed) ? (parsed as VsCodeFavorite[]) : undefined;
    } catch {
      return;
    }
  }

  private async writeFile(folder: vscode.Uri, items: VsCodeFavorite[]): Promise<void> {
