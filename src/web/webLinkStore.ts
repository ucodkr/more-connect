...
export class WebLinkStore {
  ...
  public async saveAll(items: WebLink[]): Promise<void> {
    this.items = items;
  ...
  private async readFile(folder: vscode.Uri): Promise<WebLink[] | undefined> {
    try {
      const bytes = await vscode.workspace.fs.readFile(this.fileUri(folder));
      const text = Buffer.from(bytes).toString("utf8");
      const parsed = JSON.parse(text);
      return Array.isArray(parsed) ? (parsed as WebLink[]) : undefined;
    } catch {
      return;
    }
  }

  private async writeFile(folder: vscode.Uri, items: WebLink[]): Promise<void> {
