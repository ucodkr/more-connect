...
export class DockerStore {
  ...
  public list(): DockerHost[] {
    return this.items;
  }

  public async saveAll(items: DockerHost[]): Promise<void> {
    this.items = items;
  ...
  private async readFile(folder: vscode.Uri): Promise<DockerHost[] | undefined> {
    try {
      const bytes = await vscode.workspace.fs.readFile(this.fileUri(folder));
      const text = Buffer.from(bytes).toString("utf8");
      const parsed = JSON.parse(text);
      return Array.isArray(parsed) ? (parsed as DockerHost[]) : undefined;
    } catch {
      return;
    }
  }

  private async writeFile(folder: vscode.Uri, items: DockerHost[]): Promise<void> {
